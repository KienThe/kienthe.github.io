  ...
  startCrawl: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      crawlData(crawlId, type, id, limit, page, options).catch(err => {
        ...
        crawlEvents.emit(`${crawlId}-done`, { success: false, error: err.message });
      });
...
async function crawlData(
  ...
  try {
    crawlEvents.emit(`${crawlId}-progress`, { percent: 10, message: 'Initializing crawler' });
    ...
    switch (type) {
      ...
      case 'chapter':
        ...
        break;
      default:
        throw new Error(`Invalid crawl type: ${type}`);
    }
    ...
    crawlEvents.emit(`${crawlId}-progress`, { percent: 30, message: 'Sending request to API' });

    // Gọi API
    const response = await axios.get(url, { params });
    
    crawlEvents.emit(`${crawlId}-progress`, { percent: 70, message: 'Processing response data' });
    ...
    if ((type === 'books' || type === 'chapters') && response.data.data) {
      ...
      if (options?.fetchDetails && Array.isArray(data) && data.length > 0) {
        ...
        for (const item of data) {
          ...
          try {
            const detailUrl = API_ENDPOINTS[`show${detailType.charAt(0).toUpperCase() + detailType.slice(1)}`]
              .replace(':id', itemId);
    ...
    // Complete the crawl
    crawlEvents.emit(`${crawlId}-progress`, { percent: 100, message: 'Crawl completed' });
    crawlEvents.emit(`${crawlId}-log`, {
      type: 'success',
